...
export default function BillingSuccess() {
  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        if (isSubscription) {
          const response = await axios.get(`/api/bundles/verify-checkout?session_id=${sessionId}`);
          setStatus('success');
          setBundleType(response.data.type);
          const typeName = response.data.type === 'auto_renew' ? 'Auto Renew' : response.data.type === 'upgraded_pack' ? 'Upgraded Pack' : 'God Pack';
          setMessage(`Your ${typeName} subscription is now active!`);
        } else {
    ...
    if (status === 'success') {
      const timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(timer);
            navigate(isSubscription ? '/coins/store' : '/wallet');
            return 0;
          }
          return prev - 1;
        });
  ...
  }, [status, navigate, isSubscription]);
      ...
      <Card className="w-full max-w-lg bg-[#1a1c1e] border-[#2e3337] text-white">
        <CardHeader className="text-center pb-2">
          <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-[#202229] border border-white/5">
            ...
            {status === 'success' && (isSubscription ? <Package className="h-10 w-10 text-green-500" /> : <CheckCircle className="h-10 w-10 text-green-500" />)}
            {status === 'error' && <AlertCircle className="h-10 w-10 text-red-500" />}
          </div>
          <CardTitle className="text-2xl font-bold">
            {status === 'verifying' && 'Processing...'}
            {status === 'success' && (isSubscription ? 'Subscription Active' : 'Payment Successful')}
        ...
        <CardContent className="space-y-6 pt-4">
          {status === 'success' && transaction && !isSubscription && (
            <div className="bg-[#202229] border border-white/5 rounded-lg p-6 space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-[#95a1ad] flex items-center gap-2"><Calendar className="w-4 h-4" /> Date</span>
                  <span>{new Date(transaction.date || Date.now()).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#95a1ad] flex items-center gap-2"><FileText className="w-4 h-4" /> Transaction ID</span>
                  <span className="font-mono text-xs text-white/70 truncate max-w-[150px]" title={transaction.id}>{transaction.id}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#95a1ad] flex items-center gap-2"><CreditCard className="w-4 h-4" /> Method</span>
          ...
          {status === 'success' && isSubscription && bundleType && (
            <div className="bg-[#202229] border border-emerald-500/10 rounded-lg p-6 space-y-3">
              <div className="text-center">
                <span className="text-sm text-[#95a1ad]">Active Bundle</span>
                <p className="text-xl font-bold mt-1 capitalize">
                  {bundleType === 'auto_renew' ? 'Auto Renew' : bundleType === 'upgraded_pack' ? 'Upgraded Pack' : 'God Pack'}
                </p>
                <p className="text-xs text-[#95a1ad] mt-2">Your subscription is now active. Enjoy your perks!</p>
              </div>
            </div>
          )}
        </CardContent>
        ...
        <CardFooter className="flex flex-col gap-4 pt-2">
          <Button 
            onClick={() => navigate(isSubscription ? '/coins/store' : '/wallet')}
            className="w-full bg-white text-black hover:bg-white/90 font-medium h-11"
          >
            {isSubscription ? 'Return to Store' : 'Return to Wallet'} {status === 'success' && `(${countdown}s)`} <ArrowRight className="ml-2 w-4 h-4" />
          </Button>
          {status === 'success' && (
